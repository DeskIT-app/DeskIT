# DeskIT — the owner's working notes

> This was the repository's README until 2026-09-18: every feature with
> its measurements, the config reference, the dev checkout, the owner's
> own hooks and scripts. It moved here, export-ignored, when the root
> `README.md` became the public front page. Parts of it predate the
> product's layout (keys in `.env`, `server_token.txt`, the Tailscale
> phone route): where it disagrees with `AGENTS.md`, `defaults.toml`
> or the guide in `docs/`, they are right and this is history.
...
| `punctuate_hotkey` | `ctrl+f2` | **tap** to put the punctuation into the selection — or the whole field. Whisper transcribes sounds, not sentences. `""` disables. Was `f7`, which pops Chrome's caret-browsing dialog. See [Punctuating what you just dictated](#punctuating-what-you-just-dictated) |
