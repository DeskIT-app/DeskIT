...
**D35 — The GitHub account is `DeskIT-app` (2026-09-18).** The plan was written against an organisation called `massifapp`; the day main was first pushed the owner renamed the account (he had wanted `DeskIT`, which is taken; the interim `DDeskITT` read as a typo) and, offered a probe of what was free, chose `DeskIT-app`. Consequences, all in one commit: `paths.PAGES_URL` = `https://deskit-app.github.io/DeskIT` (Pages hostnames are lowercase); `updates.REPO` = `DeskIT-app/DeskIT`; the installer's three URLs, the winget manifest's URLs, the issue templates, the guide's links and `docs/_config.yml`; the dashboard's list of hosts it will open a browser at. The winget package id stays `YoavShimron.DeskIT` — publisher, not account. The Android package id of phase 4 becomes `io.github.deskit_app.deskit` (a Java package cannot carry a hyphen; F-Droid's convention writes the underscore). GitHub redirects `massifapp/DeskIT` and `DDeskITT/DeskIT` only until someone claims those names, so nothing may keep pointing at them. Evidence: the push output "This repository moved" and his "what you find best" of 2026-09-18. Rules out: another rename after the first release — every URL is printed in the installer, the manifest and the guide by then.

**D36 — The guide is in the app: a tour of four callouts after the wizard; the site is the reference (2026-09-18).** The owner, on being offered the Hebrew guide to review: the user's guide must be minimal and in Hebrew, "not text — more like a square with an arrow, on the first start, before you use it, with a skip"; a long document nobody reads may exist beside it; and he trusts the coder's Hebrew, no review. So: `tour_card.py` + `overlay.TourCard` — four cards beside the status dot on the first start after the wizard's last page (this is the dot and its three colours; the hold key, drawn as a keycap with the person's own binding; a click on the dot opens the shelf; done — see it again from Settings > The app), one Hebrew sentence each, [הבא] / [דלג] / [סיימתי]. A stop that names the dot carries a beak on the edge facing it (the "arrow"); the window is chroma-keyed around the card so the beak stands proud. Placement is `beside_dot`, the shelf's rule. Once per copy: `setup.tour` (state key) is written on [סיימתי] and on [דלג] alike — a skipped tour that came back would be the nag the skip rules out; `App.tour_due()` shows it when `setup.done` is true, `setup.tour` false and there is a dot; the control command `tour` puts it up from the top for the dashboard's button. The wizard is unchanged: its seven pages are questions that need an answer, not lessons. The site under docs/ stays as the reference — the Hebrew index and quick start are cut to what the tour leaves a page to say (get it, install it, the key, privacy in three lines), the ten chapters sit under them as "in depth"; the English mirror is kept in step. D26's "guide on GitHub Pages, Hebrew first" therefore describes the reference, and D18 (1)'s last page gains no text — the tour follows it on the desktop. Evidence: his three messages of 2026-09-18 afternoon ("minimalist", "a square with an arrow", "take it from here"). Rules out: a tour that times out or walks by itself; a Hebrew review round before release. Revisit: after the first non-technical tester, whether four stops are the right four.

**D37 — Report v2 as built (2026-09-18, dist/phase3c): the strip only while Send is on, the row is the message, the copies travel.** Amends 7.6 and screen 7 where the build differed. (a) *The local copy is today's, always.* `record()` files the screenshot, the dictation and the whole `env` exactly as before; the four toggles (Screenshot OFF, Recording OFF, Transcript ON for `wrong`, Settings ON) are about the COPY THAT TRAVELS and nothing else, so the owner's own reports — the routine's evidence — lose nothing. The strip is drawn only while "Send to the developer" is on: a person who keeps the report here sees today's card plus one switch, and the thumbnail stays the local copy's picture whatever the Screenshot toggle says. (b) *Switches, not tick boxes*: ui.Switch's pill on the desk and the same pill painted on the hotkey card (problem_card.SWITCH_W/H). (c) *The primary's word follows the switch* — "Keep on this PC" / "Preview" — and Enter continues to the Preview when it is on (KEYS_SEND). (d) *The Preview is the desk's window, and the row is the message.* The hotkey card runs in the app's process and the desk in another, so pressing Preview on the hotkey card files the report, marks the row `sent = "preview"` with the toggles (`attach`), and opens the desk; the desk looks for such a row when it comes up or is brought forward and opens Problems with the Preview once per row. A preview he closed is a kept report with a [Preview & send] button on its row. (e) *Files first, then the row*: the Preview lists the ticked files with sizes (the picture small, Play on the wav) above the exact row, drawn line by line through DrawTextW in Courier New — a monospace face that carries Hebrew — never a tk.Text. (f) *Send copies*: `problems.queue()` writes `outbox\<uuid>.json` beside `outbox\<uuid>\` holding COPIES of the ticked files, so a local delete or purge cannot pull a queued upload apart; sb.py drops the folder when the row is up. The local row keeps `sent_id` across a resend, so the server insert stays idempotent (8.8). (g) *States on the row*: `sent ∈ {"", preview, queued, sent, failed}` with `sent_why`; the desk reads sb.py's verdicts back (`problems.sync_outbox`: payload gone = sent, `.failed` beside it = failed) on every Problems draw, and shows "waiting to send" + [Send now], "sent to the developer", or "could not be sent: <why>". (h) *The gate*: the switch asks `privacy.request("report_upload")` — directly on the hotkey card, over the pipe (`consent` command) from the desk — and stays off until `allowed()` says yes, then turns on by itself. (i) *Not built, per D33(b)*: no replies, no `developer_status`, no Fixed press-back. Evidence: `test_the_copy_that_travels_is_the_toggles_and_nothing_else`, `test_the_report_cards_send_switch_asks_its_card_first_and_answers_with_the_toggles`, `test_send_to_the_developer_previews_before_anything_leaves`. Rules out: a strip on the plain card; a toggle that thins the local report. Revisit: (a) if strangers ask for the local copy without the picture — a [problems] shot switch already exists.
...
**D29 — Open decisions only the owner can settle (each with the recommendation).** (1) Licence: Apache-2.0 (recommended) vs FSL. (2) Public name/identity: keep "DeskIT" and reserve the trademark; package id `io.github.deskit_app.deskit`. (3) Store timing: submit in phase 3 (recommended) vs skip the Store entirely. (4) Anonymous-first accounts (recommended) vs e-mail from day one (needs a domain earlier). (5) Whether the Saturday routine keeps processing strangers' reports with Claude under his account (recommended yes, disclosed in the policy) or he reads them by hand. (6) Spend USD 25 on Google Play in phase 4 (recommended) vs GitHub APK only. (7) Buy a domain for e-mail linking (phase 5). (8) Whether the English detector stays a feature at all (recommended: optional on GPU tier only).
...
| `Prefs.DEFAULT_URL = "https://your-pc.your-tailnet.ts.net"` is the default server of every APK install; package id `com.yoav.dictation` | `Prefs.kt:8-11, 25`; `build.gradle.kts:7, 11`; `method.xml:3`; `Notify.kt:34`; `tools/emu.py:30` | a stranger sees the owner's hostname pre-filled (`SettingsActivity.kt:77`; `HomeActivity.kt:180-181`) |
...
| 10 | GitHub `origin` (`DeskIT-app/DeskIT`) | `git fetch/push origin main`, `git reset --keep origin/main` (`dashboard.py:1031-1039, 1085-1104`) | owner's git credential helper | the code, and any untracked-but-not-ignored file (`questions.json` today) | Push/Undo buttons | owner-only | n/a |
...
8. *Written 2026-09-18 (`supabase/migrations/0001_init.sql`, `sb.py`, `sync.py`):* the `where` column is `place` (a reserved word); `history`'s key is `(user_id, device_id, ts, kind)`; `vocab_sync` rows carry `deleted` as the tombstone; no `report_replies` (D33); `status` is `open | fixed | closed`; the machine-bound keys are decided by `sync.syncable()` in code, not by a schema flag (the settings schema has no per-key metadata to carry one); the publishable key rides as the `apikey` header attached by `net.py` for the one host; open point 2 resolved as REST through `net.py`; open point 4 resolved as "the client purges its own folder first, the RPC deletes the rows"; open point 1 (Turnstile) left OFF — anonymous sign-ins rely on the default rate limits until abuse is seen.
...
and `/CHANNEL=` (this chapter's two additions), `/LANG=hebrew|english`, and `/NODOWNLOAD` (below).

*2026-09-19 (the owner, after three stranger walks: "everything comes the moment I install; no
installation page"): the installer downloads what chapter 9's computer page used to.* A **Downloads**
page after Welcome lists what this PC still needs, each a ticked box with its size and its licence
links: the Hebrew model and the Recording pack always; NVIDIA's CUDA libraries with a card of 4 GB
and a driver from 545.84 (`nvidia-smi` through `cmd`, the same question hardware.py asks); the
English detector with 6 GB. `TDownloadWizardPage` fetches the ticked items file by file into `{tmp}`
against the locks' SHA-256s — the same URLs and hashes models.py and packs.py use, from
`packaging\downloads.iss`, GENERATED by `dev\make_downloads_iss.py` out of models.lock, packs.lock and
defaults.toml and committed (both builds run `--check`). At ssPostInstall the files move under
`%LOCALAPPDATA%\DeskIT\models\…` and `\packs\<name>\wheels\`, and the installed Python runs
`main.py --adopt-downloads`: `models.adopt` hashes and writes `.complete`, `packs.adopt` runs pip
`--no-index` on the wheels — no window, no network, exit 0 always. What is on disk already (the
marker holds the lock's revision / every wheel's version) is not offered; a failed download is Retry
...
| Guarantee | Mechanism |
|---|---|
| `DATA_DIR` is never touched | the installer's `[Files]` and `[InstallDelete]` sections name only `APP_DIR`; a build-time test greps the Inno script for `localappdata\DeskIT` outside the uninstaller's optional "remove my data" step (chapter 10). *Since 2026-09-19 the one exception is the Downloads page (10.4): a ticked download is PUT under `DATA_DIR\models\…` or `\packs\<name>\wheels\` — added, never listed or deleted — and a silent run, which is what the Update card starts, never reaches it; the test names the two lines that may say `{localappdata}` and the two that may use them.* |
...
| Package `com.yoav.dictation` | `io.github.deskit_app.deskit` (12.6) |
...
rename is the first Android commit of phase 4 and precedes any APK given to anyone. Target: `io.github.deskit_app.deskit`
...
| `android/app/src/main/java/com/yoav/dictation/**` | all | Directory moves to `io/github/deskit_app/deskit/`; package declarations follow |
| `Notify.kt` | 34 | `ACTION_DECIDE = "io.github.deskit_app.deskit.DECIDE"` |
...
| `test_apk_id_matches_decision` | `android/app/build.gradle.kts`, `method.xml`, `Notify.kt` and the Play metadata all carry `io.github.deskit_app.deskit`; `com.yoav.dictation` appears nowhere in the tree. |
...
| 0.2 | Decide the public name and the Android package id (Part B, decision 2). | — | 0 | `applicationId` in `android/app/build.gradle.kts:11` (today `com.yoav.dictation`) becomes `io.github.deskit_app.deskit` before any outside install; it cannot change afterwards (D22; chapter 12). |
| 0.3 | Rotate and delete his own API keys: the Gemini, Google, Cerebras and Groq keys that `apikey.py:24-26` reads from `.env` today. Delete them in the Google AI Studio and Groq consoles, close the Cerebras key entirely (the path is removed, D6), then create fresh personal keys used only for testing. Optionally enable Zero Data Retention on his Groq console. | aistudio.google.com, console.groq.com, cloud.cerebras.ai | 0 | Nothing enters the repo: from phase 1 the test keys live in Credential Manager under `DeskIT/groq` and `DeskIT/gemini` (D3). Google's API ToS forbids embedded developer credentials (`research/keys-privacy` "What the owner must do"). *DONE 2026-09-19 (the owner, by hand): every old AI Studio project deleted and one new project with one new Gemini key; a new Groq key and the old one revoked; the Cerebras key deleted (the console refuses to delete the only key, so a never-copied `unused` key stands in its place) and its `.env` line removed. `.env` now holds the two fresh keys only; still `.env`, not Credential Manager — 1.3 remains.* |
...
| 0.5 | Check git history for private strings before publishing. Known today: the tailnet host `your-pc.your-tailnet.ts.net` is a tracked constant at `android/app/src/main/java/com/yoav/dictation/Prefs.kt:25`, so it is in every commit that touched that file; the phone token that once sat in `app.log` is untracked but must be confirmed absent. Decide between publishing from a fresh history and rewriting (Open points). | the checkout | 0 | — (decision reported in chat; the coder deletes `DEFAULT_URL`, D22). *2026-09-18, lane B: the whole history scanned (249 commits, 1,486 blobs, every ref) — no key, token or record of the owner's was ever committed, only test fixtures; the tailnet host is the one identifier, in `Prefs.kt`, this plan and two maps, still in HEAD; `shimr` in absolute paths and the once-tracked `config.toml` (settings only) are the rest. The owner's decision: edit HEAD only, no rewrite — lane A executes.* |
...
| 0.7 | Run the phase-0 checks the coder prepares: VirusTotal scan of a throwaway embeddable-Python installer (needs Inno Setup 6 and a full Python 3.11 on the build machine, `research/packaging` "What the owner must do"); Supabase anonymous sign-in + Turnstile availability on Free; existence of an LGPL cp311 `av` wheel (D24, D27). | virustotal.com, supabase.com, github.com/JScheumann/pyav-ffmpeg-lgpl | 0 | Results reported in chat; they decide chapter 10's Defender FAQ, chapter 8's auth fallback and chapter 6's Recording pack. *2026-09-19: `VT_API_KEY` is a repository secret (the owner's own VirusTotal account); the first real scan happens on the next `release.yml` run. Anonymous sign-in on Free: verified live by lane A (phase 3). Turnstile: NOT enabled — `sb.sign_in_anonymous()` sends no captcha token (`sb.py:583`), so turning it on would refuse every first Send; the Pages-widget round-trip of chapter 8's Open point 1 is the prerequisite.* |
...
| 1.2 | Keep `DESKIT_SUPABASE_SECRET` only in his user environment, never in `.env` or the repo (D17, D28). | Windows environment variables | 0 | — *DONE 2026-09-19: the project's `sb_secret_` key (the dashboard's `default`) is a User variable on the owner's PC, set through the Environment Variables dialog; verified present, never printed. `dev/inbox.py` and `test_migration_matches_dashboard` can run here now.* |
...
| 3.4 | Authentication: enable anonymous sign-ins and Cloudflare Turnstile (verified in 0.7); leave the e-mail provider for phase 5; set Site URL to the GitHub Pages guide (D17). | Supabase dashboard | 0 | — *2026-09-19, checked in Studio: anonymous sign-ins ON; Site URL = `https://deskit-app.github.io/DeskIT`; redirect list holds `http://127.0.0.1:*/**`. Turnstile deliberately OFF (see 0.7).* |
...
| 3.6 | Turn on MFA for the Studio login; add the weekly keep-alive GitHub Actions cron (one REST GET with the publishable key) so the Free project does not pause after 7 idle days; schedule a monthly `pg_dump` through the pooler connection string (Free has no backups) and a monthly glance at DB size vs 500 MB and storage vs 1 GB (D17). | Supabase, GitHub | 0 | `.github/workflows/keepalive.yml`. *2026-09-19: MFA ON for the Studio account (one authenticator app, Google Authenticator; a second factor is still advised). Keep-alive: `supabase-keepalive.yml` exists, its two repository variables set 2026-09-18. First `pg_dump` taken by hand through the session pooler (`aws-0-eu-central-1.pooler.supabase.com:5432`, user `postgres.<ref>`, password prompted, never on the command line) → `Documents\deskit-2026-09.dump`, 339 KB, `pg_restore --list` reads it. Monthly: the same line with the new month in the file name.* |
...
| 2 | Public name and identity | Keep "DeskIT", reserve the trademark, package id `io.github.deskit_app.deskit` (D22, D23) | A different name means new Store and Play reservations and a new package id; the package id is the one string that can never change after the first outside install. | Phase 0 for the id; phase 3 for the Store name |
...
| Test | Asserts |
|---|---|
| `dev/test_public_tree.py::test_no_owner_strings_in_archive` | The `git archive` tree contains none of: the owner's tailnet host (the `ts.net` name `Prefs.DEFAULT_URL` carried until 2026-09-18), the old package id `com.yoav.dictation`, `Yoav.DeskIT` as an AppUserModelID, a `.env` file, `questions.json`, `questions.lock`, `.agents/`, `portable.txt`, or any string matching the redactor's key patterns (D8, D22, D28). |
...
- **Git history.** D28 says "check git history for the tailnet name/token". The check is already
  ...
  file, so `your-pc.your-tailnet.ts.net` is in the history of every commit since it was added. Deleting
