...
| `reading.py` | read this to me: the Corrections place's Read aloud tab. One sentence on a card, read into the dictation key with the dashboard in front, and filed under the CARD'S words in corpus\read (beside the corpus, so corpus_keep never trims it). THE DECK IS PROSE, READ IN ORDER: any .txt/.md he drops into corpus\read\texts, and when it runs dry a paragraph the polish backends write on the next of `SUBJECTS` (`Writer`, Groq then Ollama, text only, ~1 s, saved as written-<stamp>.txt like any other file); `plausible` keeps a lone letter, glued punctuation or a triple word off the card. THE ENGLISH ON A CARD IS NOT CHECKED (`match` forgives a word with Latin or digits in it whatever came back -- "קומיט" for commit): the model knows English, what it cannot do is write commit in Latin letters inside Hebrew, and a reading labelled "commit" is the pair that teaches it; only the Hebrew has to come back as written. At most a third of a written paragraph carries a term (`mixed`), round the names in vocab.json (`names_of`). His decision, 2026-09-13 evening, after an hour of Hebrew-only. His own dictations were the first source and were dropped on 2026-09-13 -- a dictation is not prose. Kept and skipped sentences (skipped.json, the sidecar's `key`) are never offered again. The app owns the mic and the models: `_on_start` decides `_to_read` at the press from the foreground window alone, `_handle` diverts the transcript -- after the whole repair a dictation gets, vocab and the context pass both (it went without the pass for an evening and the card asked about slips the pass fixes on every dictation) -- to `Reading.heard` instead of pasting, the `read` control command is arm / disarm / keep / drop and `status()["read"]` is what the tab draws. Word for word back = kept without asking and the next one comes up; anything else asks him (keep / read again / skip). Two bars in [study]: read_minutes, read_goal_hours |
