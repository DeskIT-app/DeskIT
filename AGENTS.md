...
Claude (awake.py, Settings › General › Awake, `ctrl+alt+n`), a notify
...
- **Settings** — eight tabs since 2026-09-22: General, Dictation,
  Screen, Messages & sounds, Phone, Account, Privacy, About
  (settings.py: TABS names the forty-odd lines the screen draws, with
  their plain words; every other line of defaults.toml is a measurement
  the developer edits in the file and the screen never shows — the
  owner, 2026-09-18: "I am the user; you are the developer". A path is
  named by exactly one tab, a test holds it). `dashboard.LAYOUT` says
  what each tab draws and in what order, cards and rows together.
  General is this computer and this screen (the dot, the microphone,
  the two save folders, Start with Windows, updates, Awake), Dictation
  the words (the engine, the speech model and the graphics card, the
  switches), Messages & sounds what other programs say here plus every
  cue, Account the sign-in, the two syncs and the lock, Privacy the
  keys, the gates, the connections and what is kept, About the version,
  the papers and the app's own files. His brief: "I was looking for
  where the videos and the pictures are saved and could not find it …
  either put them all in General, or make more tabs at the top". No
  sentences, no "Everything", no fold, no Text/Cards/Speed tabs — each
...
- **The bar's buttons follow the state and nothing else**
  ...
  **Since 2026-09-18 Stop unloads the MODEL, not the process**
  ...
  The whole
  process quits from Settings > About > Quit DeskIT or the shelf —
...
- **Every personal-store path comes from `paths.py`, never from
  ...
  the released copy can run beside it; `DESKIT_STRANGER=1` with a `DESKIT_HOME`
  runs this same checkout as an installed copy (`dev\stranger.py`, the row
  below), which is how a first-run bug is reproduced HERE and not on the
  installed copy. A new store gets a row in
...
- **A `PhotoImage` that outlives its interpreter aborts the process, and it
  ...
  you this.
- **A window built BEFORE the app starts is buried on the main thread
  before the app starts.** The wizard (firstrun.py), the model and the
  pack steps run on the main thread and close before `app.start()`;
  their widgets, PhotoImages and callbacks are reference cycles that
  only the cyclic collector frees — on whichever thread trips the
  allocation threshold next, which after start is a DECODE thread. The
  stranger's copy died 34 s into its first dictation on 2026-09-19 with
  exactly the abort above in spawn.log. `firstrun.run()` drops the wizard
  and collects twice before returning, main.py collects once more before
  the app, and `test_the_wizard_leaves_no_tk_object_for_another_thread_to_bury`
  counts survivors WITHOUT collecting (43 without the burial, 0 with).
...
- **A Tk screen switch is paid per WIDGET, at first map, and every
  `update_idletasks()` in a builder is a frame the person sees.** Measured
  2026-09-19 on the hidden desktop, raw Tk: the first map of a widget costs
  1.3-2.3 ms on this PC (one OS window each — 92 Frames 0.12 s, 92 Labels
  0.22 s), a fresh PhotoImage of any size costs nothing, a repaint of a
  screen that exists costs 10 ms. The desk's `_show` therefore unmaps the
  sheet (`place_forget`) while the old screen is destroyed and the new one
  built, and `_slide_in` puts it back — the builders' `update_idletasks()`
  calls still lay out (reqheight arithmetic works unmapped; pixels of all
  seven screens identical) but paint nothing, where before Home showed
...
- **A glow that does not reach alpha 0 inside its window IS the window.**
  ...
  inside its box (the dot's halo ends at `HALO_R` = 18 px, alpha 0 on the
  whole border; the mark tile stood in for the dot for one afternoon on
  2026-09-19 and the owner asked for the disc with its halo back that
  evening — `skin\mark.py` still draws the boot card's badge), and
...
| `weekly_review.ps1` + `.claude/commands/weekly-reports.md` | the Saturday 08:00 review of the bug list: a scheduled task runs the wrapper, the wrapper runs the local `claude.exe` on the project command, and the command reads `problems.json`, writes the summary, the plan and the archive into `problems\weekly\`, closes only what it understood, regenerates `problems.md` and posts a card through notify_hook.py. It builds only what he answered, and it asks rather than guesses — a report it cannot explain stays open with a question against it. The command is committed (it is the routine); everything it writes is gitignored (it is his). **Since 2026-09-23 (audit A13) the wrapper pulls the inbox itself, takes every key and token out of claude.exe's environment, and runs two parts under `--permission-mode dontAsk` with explicit lists**: the owner's part (his reports and answers; may edit and commit, never push, never read `problems\inbox`) and the inbox part (other people's reports; reads, and writes `problems\weekly\<date>-inbox.md`, nothing else — a stranger's report is described, never built). It never starts claude as `/weekly-reports`: the command's `allowed-tools` frontmatter widens `--allowedTools` (measured with 2.1.280), so the run reads the file instead |
...
| `foreground.py` | the way back after a Google sign-in (2026-09-19): `bring_back(title_part)` finds the app's own Tk top-level (the wizard or the desk, never the browser tab of the same title) and tries SetForegroundWindow, AttachThreadInput + BringWindowToTop, minimise/restore, then FlashWindowEx; `signed_in_card(email)` posts "Signed in as <email> · Back to DeskIT" through POST /notify with the window's hwnd so the click raises it. sb.py calls both right after the code exchange; the sign-in page it serves is `sb.signin_page(ok, why)` — LAMPLIGHT, the mark as a data URI, no external request |
...
| `packaging\DeskIT.iss` | the per-user Inno installer (10.4): no UAC, Hebrew/English by the Windows language, Restart Manager on upgrade, downgrade refused, `/CHANNEL=` and `/NOLAUNCH`, the uninstall question defaulting to Keep. **Since 2026-09-19 it downloads what the wizard's computer page used to** (the owner: "everything comes the moment I install; no installation page"): a Downloads page after Welcome — the Hebrew model and the Recording pack always, the CUDA libraries with a card of 4 GB and a driver from 545.84 (`nvidia-smi` asked the way hardware.py asks), the English detector with 6 GB — each a ticked box in PLAIN WORDS ("Hebrew speech — 1.6 GB", "Faster with your NVIDIA card — 1.4 GB"; his rule of that night: a page of exact figures, library and licence names reads like a programmer's and makes half the people MORE afraid — the details go behind one "More about these downloads" link to the guide's `01-install#downloads` table, which holds every licence link), `TDownloadWizardPage` against the locks' SHA-256s into `{tmp}`, placed under `%LOCALAPPDATA%\DeskIT\models\…` and `\packs\<name>\wheels\` at ssPostInstall, then `python\python.exe app\main.py --adopt-downloads` (models.adopt / packs.adopt: hash + `.complete`, pip `--no-index`; never the network; exit 0 always). An item whose marker holds the lock's stamps (the revision; each wheel's version) is not offered; `WizardSilent` and `/NODOWNLOAD` download nothing; a failed download is Retry or the wizard's, never a failed install. The list is `packaging\downloads.iss`, GENERATED by `dev\make_downloads_iss.py` from models.lock + packs.lock + defaults.toml and committed; both builds run its `--check`. Compiled on this PC with Inno Setup 6.7.3 (per-user, `%LOCALAPPDATA%\Programs\Inno Setup 6`, installed 2026-09-19 through winget; build_local.ps1 looks there too) and the Downloads page photographed on the hidden desktop in both languages (`dev\shot_installer.py --next 1`: four items on this PC's card, the links mirrored under Hebrew). Not yet walked to the end for real: the download itself, the placing and the adopt step inside a real install are the next hands-on proof |
...
| `firstrun.py` | the first-run wizard, eight pages in one 720x720 window (chapter 9.2, `docs/distplan/research-onboarding.md`): Welcome (three privacy sentences, the guide's "check it yourself" link) → Microphone (the live meter, Windows' privacy switch read from the registry before the meter is even drawn, the trap named with the fix one click away) → This computer (the probe's line; the Hebrew model as a `StepPane`, the GPU pack, the English detector and — since 2026-09-19 evening, on by default — the Recording pack as switches that queue behind it — one [Download], one bar, "1 of 3 · Hebrew model"; the queue keeps running while the wizard moves on and is drawn again on the next page) → Say one sentence (the record button waits for the model; the measured seconds into `hardware.last_test_seconds`) → Keys → Optional extras (D33's five: cloud repair — THE SWITCH IS THE CONSENT here (the owner, 2026-09-19 evening: "whoever turns it on, that is enough"; privacy.grant with the card's text_version), and on it opens a rounded `ui.Field` for the Groq key under the row, checked with one `key-test` request the moment it is saved (`key_probe`); a refused key leaves the store, and Next waits while the switch is on without a working key (`_extras_gate`) — keep-awake, weekly update check, Connect Claude Code, the Snipping-Tool key; drawn as they stand, written only when moved) → Ready (the hotkey, Start with Windows and the phone asked once, and — signed in — **Keep my learned words and settings in my account**, ON by default since 2026-09-20 (the owner: nobody found the gate in Settings — "make it the default, on the last screen, with a line that says what they are ticking and how to turn it off"); the row's help line says what follows the person and that Settings > Privacy > Withdraw stops it; `_save_sync` records `privacy.grant("settings_sync")` with the card's text_version on [Start] ONLY — never on the extras page's Next, never when the row was not on the page (no account: a consent nobody saw is not one), and off on a later run with the gate open withdraws it; the app's worker then syncs 30 s after start; [Start] starts the app AND opens the desk — the separate [Open the desk] went on 2026-09-19 evening, after the owner pressed Start without it and met a dot with nothing to look at). `setup.done` into state.json (`record_done`; the marker file only for `--config` runs); `run()` returns a `Result` (truthy when saved; `open_desk`, `installed_pack` for main). Imports nothing from main.py. Chrome English, every paragraph Hebrew through `ui.draw_text`; the string table is `WORDS` |
| `tour_card.py` + `overlay.TourCard` | the guide itself (D36): four callouts beside the status dot on the first start after the wizard — the dot and its colours, the hold key drawn as a keycap, what a click on the dot opens, done — one Hebrew sentence each, [הבא] / [דלג] / [סיימתי]. Split like consent_card.py: the words (`STOPS`), the geometry and the picture in `tour_card.py` (pure Pillow; `frame` adds the beak's room on the side that faces the dot, `clamp_tail` keeps it off the corners, `flat` paints card + beak on the chroma colour the window keys out); the thread, the queue and `place` (beside_dot, then which edge faces the dot) in `overlay.TourCard`. Once per copy: `setup.tour` in state.json, written by `main.App._tour_ended` on [סיימתי] AND on [דלג] (a skipped tour never comes back by itself); `App.tour_due()` decides at start; the control command `tour` and Settings > About > Show the tour put it up again. The docs site is the reference, not the guide |
...
| The speech model on Settings > Dictation, the Home rows | the four `[local]` knobs the probe writes and `setup.offer_gpu_pack` are the file's, not the screen's (the Speed tab went 2026-09-18, THIS PC on The app 2026-09-22); `dashboard._model_block` = the model's standing and the graphics card's, in plain words ("The Hebrew model: ready (1.62 GB)", "Faster dictation: on — the work runs on your NVIDIA card"), `hardware.machine_line()` on About with the buttons each state earns (download / continue / delete and re-download / delete; turn on / update / retry / reinstall / remove — each step in a process of its own through `launch.run_step`); `dashboard._waiting_hardware` = the Home pile rows for a model not there, a pack that will not run (`failed:`), a tier that changed (`hardware.tier_changed`, cleared by OK); `_voice_block` on Screen when the probe found no he-IL voice (`hardware.no_voice`, `apply_voice` puts `visual_qa.speak = "off"` in the machine layer); provider menus drop "On this computer" when `hardware.ollama_absent()` unless it is the held value |
...
| `sb.py` | the account (chapter 8, D17, D31): the one server DeskIT runs, spoken to only through `net.py` by purpose (`account`, `sync`, `history`, `report`). Two public constants — the project ref and the PUBLISHABLE key (`PROJECT_REF` empty until the owner's project exists; every function then answers "not configured"). Google sign-in through Supabase's PKCE flow with a one-shot listener on 127.0.0.1 (`_listen` / `_wait_for_code` / `_exchange`), or an anonymous account, linkable later; the session as one DPAPI blob under the name `supabase_session` (net.py reads the access token out of it by name); one refresh on a 401, a second 401 drops the session. `sync_now` = pull/merge/push for the three stores behind their gates; `drain_outbox` = the queued reports, objects before the row, a 4xx marks `.failed`, a 5xx waits; `start_worker`/`nudge(store)` = the app's thread (30 s after start, every 15 min, 0.5 s after a nudge — over the store the nudge named: `history` after a dictation, `vocab` after a learned word). **The live channel (2026-09-20, the owner: "not two seconds"):** every push ends with `_broadcast` — one REST POST on the account's private Realtime topic `user:<uid>` carrying the store names and this device's id, nothing said — and `_live_loop`/`_live_once` (thread `account-live`, `LIVE_ENABLED`, off in tests at import) holds one `net.websocket` on that topic, joins it with the session token by name (`send_text(..., secret="supabase_session")`), heartbeats every 25 s, re-sends a refreshed token, and on a broadcast from another device runs `sync_now(only=stores, push=False)`; refused or dropped, it waits `LIVE_RETRY_S` and tries again; `status()["live"]` is the Account row's "live". Migration 0003 is the pair of policies on `realtime.messages` that keep one account's topic from another. The dashboard never holds the session: it sends `account` over the pipe (`main.App._account_command`) and reads `status()["account"]`. A static test keeps its imports narrow — never a module that reads a cloud key, never a provider's name. **`REQUIRED = True` is the owner's rule of 2026-09-18: no account, no dictation.** `main.App.locked()` = a configured project and no session → `_lock()` holds the state machine paused with the sign-in sentence (no pause cue; the pause key and the dashboard's Resume put it straight back; the phone is refused), `_unlock()` after a sign-in over the pipe, `sb.SIGNED_OUT_HOOKS` locks again when the session goes (Sign out, Delete, a second 401). The wizard's `account` page (firstrun.py, page 1) has no way past without a session; since 2026-09-20 it is a stack of its own (`ACCOUNT_STEPS`: choice → create | signin → signed; `_account_show`, `_account_back`) — Create an account (a required name, `set_name` keeps it in the auth user's own metadata, then the whole wizard) or I have an account (the sign-in, and when the account holds settings the words come down and the desk opens with nothing more to press); Back walks the stack before it leaves the page, and on the signed card it is Not you? — `sign_out(everywhere=False)`, this PC only. **And the desk (Q1, 2026-09-18 night): `dashboard._locked_now()` — the app's `status()["locked"]` when it answers, sb.py's own session when it does not — puts the window on the LANDING (`_landing`/`_screen_landing`: the places off the bar, one card, [Sign in with Google], every `_show` refused) until a session exists; the button goes over the pipe to a running app, or runs `sb.sign_in_google` in the window's own process when there is none.** tests.py sets `REQUIRED = False` at import; the lock's tests set it back |
| `vault.py` | the account lock (2026-09-21, the owner: what he said and the cloud keys locked before they leave, the same account opens them on every PC, the server cannot). One random key per account in a DPAPI file (`secretstore` name `account_key`, bound to the uid; `key`/`create`/`keep`/`forget`, `fingerprint` = 16 hex of a hash, what `profiles.lock_id` holds); AES-256-GCM through Windows' own CNG (`bcrypt.dll`, ctypes, no package: `seal`/`open_`, a fresh nonce per value, the row's identity as the associated data — `AAD_HISTORY`/`AAD_VAULT`/`AAD_PAIRING`/`AAD_RECOVERY`); the hand-off to a new PC as Apple and Bitwarden do it — the applicant's ECDH P-256 pair (`applicant(pairing_id)` keeps the private half in memory), the holder wraps the key to its public point (`hand_over`: ephemeral ECDH, HKDF-SHA256, one sealed value), the applicant unwraps (`take`); a generated recovery key (`new_recovery`, 24 characters of Crockford's alphabet without I/L/O/U, six groups; `wrap_recovery`/`unwrap_recovery`, scrypt 2^17 — half a second, 128 MiB) and the eight-character pairing code (`new_code`, `normalize`, `pretty`); formats `v1.`/`p1.`/`w1.`/`r1.` + standard base64 (`is_sealed` is the migration's `is_sealed()` verbatim). **The narrow door for the cloud keys' values**: `export_keys` (Credential Manager only, never .env or the environment) → sealed rows, `import_keys` → Credential Manager, `digest_keys` → keyed hashes and which names hold a value; sb.py sees sealed text only (`test_sb_imports_are_narrow` holds vault.py's own imports to the stdlib + secretstore and sb.py off every other secret). `test_vault_seals_hands_over_and_recovers_through_cng` is the round trip; `test_the_lock_is_made_once_and_a_second_pc_joins_by_approval_or_recovery` the three-PC story |
| `sync.py` | what leaves the PC for the account and what never does (D31), no network in it: `syncable()` (the [privacy] gates, the phone endpoint, devices, folders, hotkeys, corners, positions, ports, the hardware tier, the local and Ollama models stay home — `test_sync_serializer_drops_sync_false`), the settings blob and its merge (last writer wins, ≤ 64 KB), `vocab.json` as rows (union by what was heard, higher hits wins, a tombstone for a word forgotten since the snapshot; study.py's hits-0 guesses never travel), `transcripts.log` events as rows keyed to the millisecond — two events of one kind on one stamp are moved a millisecond apart, deterministically, since 2026-09-20, when the whole-second key had every batch with such a pair refused (HTTP 500) and the owner's Dev copy pushed nothing for two days — and the other PCs' rows back as `REMOTE` lines in `sync\history.log` (history.py folds them into the Said page as "from <device>", a learned row as `shown || fixed` so its pair reaches Lately); the cursors in `sync\cursor.json` |
| `supabase/migrations/0001_init.sql` + `supabase/README.md` | the whole schema of the account server, pasted once into the project's SQL editor, never a dashboard edit (8.13): seven tables (ten since `0004_account_lock.sql`: `vault`, `recovery`, `pairings`, `profiles.lock_id`, `history.cipher` in place of `text`/`raw` — every opaque column under `is_sealed()`, whose length cap `0005_is_sealed_within_postgres_limits.sql` moved into `char_length` because Postgres caps a regex bound at 255 (0004's `{16,7600}` refused every call, measured live); the plaintext history rows deleted; D12 lock 3 reads "no column the server can READ holds a key" since then; `tests._sql_tables` folds the `alter table add/drop column` of every migration into the columns it checks), RLS on each, every policy `to authenticated` with `(select auth.uid()) = user_id`, `anon` revoked from everything, the private `reports` bucket with its limits and three object policies, `delete_me()`; no column can hold a key — every text/jsonb column carries `looks_like_key()`, whose patterns are `redact.PATTERNS` verbatim in the header block (`test_redactor_patterns_match_migration`), and `problem_reports.env` is `problems.env()`'s whitelist (`test_env_matches_server_whitelist`). No `report_replies` (D33). The README is the sceptic's ten-minute read. Export-ignored: it is the repo's proof, not the app's file |
