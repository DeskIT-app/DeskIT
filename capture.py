...
class ClipBar:
    ...
    def _poll_hover(self) -> None:
        """Hover by POSITION, not by event.

        The bar used to open on Tk's <Enter> and close by polling the
        pointer. Measured 2026-09-12 11:36:33, with the dictation key
        latched: the pointer sat inside the pill's own window — Windows
        said so, WindowFromPoint answered the pill's TkChild — and no
        <Enter> ever came, so the bar stayed shut until a drag jolted Tk
        into a fresh Enter. An Enter that Tk owes and never delivers
        cannot be waited for. The same poll that closed the bar now opens
        it: the pointer inside the pill IS the hover, whatever Tk thinks
        it saw. <Enter> stays bound; it merely stops being the only door.
        """
        inside = self._pointer_inside()
        if inside:
            self._hovering = True
        elif self._hovering and time.monotonic() - self._left_at > 0.28:
            self._hovering = False
