...
"שאל על המסך" דרך Groq עובד שוב (21.9.2026). המודל שהעותק שלח אליו את צילום המסך, qwen3.6-27b, ירד מהקטלוג של Groq — כל שאלה שהגיעה אליו חזרה עם 404, שורה אחת ביומן, והשאלה המשיכה למאגר היומי המשותף של ג'מיני בלי שמישהו ידע למה. הברירה עכשיו היא qwen3.8-27b, המודל היחיד שם שמקבל תמונה (נמדד מול הקטלוג החי: כל השאר עונים "content must be a string"); עונה תוך 0.3–0.5 שניות, וקורא את הטקסט שעל המסך מילה במילה. הקצבה לא השתנתה: שני צילומי מסך בדקה, השלישי ממתין.
